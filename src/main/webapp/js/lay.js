...
layui.define(['jquery','layer','element','common'],function(exports){
     ...
     Navbar.prototype.render = function(){
     	...
     	if(typeof(_config.elem) !== 'string' && typeof(_config.elem) !== 'object'){
     		common.cmsError('Lay Nav error: elem参数未定义或设置出错','elem参数出错');
     	}
     	// 若为字符串
     	var $container;
     	...
     	if ($container.length === 0) {
            common.cmsError('Lay Nav error:找不到elem参数配置的容器，请检查.', '容器参数配置出错');
        }
        if (_config.data === undefined) {
            common.cmsError('Lay Nav error:请为Nav配置数据源.', '数据源配置出错')
        }
        // data处理
    ...
    Navbar.prototype.on = function(events,callback){
        ...
        if(typeof(events) !== 'string'){
        	common.cmsError('Lay Nav error:事件名配置出错，请参考API文档.','事件名配置出错')
        }
         ...
         for (var i = 0; i < data.length; i++) {
            ...
            if(data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0){
            	...
            	if(data[i].icon !== undefined && data[i].icon !== ''){
            		if(data[i].icon.indexOf('layui-') !== -1){
            	...
            	for(var j = 0; j < data[i].children.length; j++){
            		...
            		if(data[i].children[j].icon !== undefined && data[i].children[j].icon !== ''){
            			if(data[i].children[j].icon.indexOf('layui-') !== -1){
     ...
     exports('lay',function(options){
     	 return lay.set(options);
     })
})