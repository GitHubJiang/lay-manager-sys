...
function changeDetail(v){	
    var str="0";
    for(i=0;i<v;i++){ str+=("_"+(document.getElementById(sDetail[i]).selectedIndex-1));};
    var ss=document.getElementById(sDetail[v]);
    with(ss){
        length = 0;       
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(sDetail[v-1]).selectedIndex>0 || !v)
        {
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                for(i=0;i<ar.length;i++)options[length]=new Option(ar[i],ar[i]);
        ...
        if(++v<sDetail.length){changeDetail(v);}
    }
}
function changeModify(v){	
    var str="0";
    for(i=0;i<v;i++){ str+=("_"+(document.getElementById(sModify[i]).selectedIndex-1));};
    var ss=document.getElementById(sModify[v]);
    with(ss){
        length = 0;       
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(sModify[v-1]).selectedIndex>0 || !v)
        {
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
        ...
        if(++v<sModify.length){changeModify(v);}
    }
}
function changeAdd(v){	
    var str="0";
    for(i=0;i<v;i++){ str+=("_"+(document.getElementById(sAdd[i]).selectedIndex-1));};
    var ss=document.getElementById(sAdd[v]);
    with(ss){
        length = 0;       
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(sAdd[v-1]).selectedIndex>0 || !v)
        {
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                for(i=0;i<ar.length;i++)options[length]=new Option(ar[i],ar[i]);
...
dsy.add("0_1",["北京"]);
dsy.add("0_1_0",["门头沟区","通州区","大兴区","怀柔区","昌平区","丰台区","房山区","东城区","宣武区","西城区","朝阳区","平谷区","密云县","石景山区","海淀区","顺义区","崇文区","延庆县"]);
...
dsy.add("0_12_1",["长沙县","宁乡县","望城县","浏阳市","芙蓉区","天心区","岳麓区","开福区","雨花区" ]);
...
dsy.add("0_23",["上海"]);
dsy.add("0_23_0",["青浦区","松江区","金山区","浦东新区","嘉定区","宝山区","闵行区","杨浦区","虹口区","普陀区","静安区","长宁区","徐汇区","黄浦区","奉贤区","闸北区","卢湾区","南汇区","崇明县"]);
...
var sDetail=["provinceDetail","cityDetail","districtDetail"];
var sModify=["provinceModify","cityModify","districtModify"];
var sAdd=["provinceAdd","cityAdd","districtAdd"];
var opt0 = ["-","-","-"];
function setup()
{
...
{
    for(i=0;i<sDetail.length-1;i++)
        document.getElementById(sDetail[i]).onchange=new Function("changeDetail("+(i+1)+")");    	
    changeDetail(0);
}
function setupModify()
{
    for(i=0;i<sModify.length-1;i++)
        document.getElementById(sModify[i]).onchange=new Function("changeModify("+(i+1)+")");    	
    changeModify(0);
}
function setupAdd()
{
    for(i=0;i<sAdd.length-1;i++)
        document.getElementById(sAdd[i]).onchange=new Function("changeAdd("+(i+1)+")");    	
    changeAdd(0);
}