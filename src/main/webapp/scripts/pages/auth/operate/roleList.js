...
wms.addReadyFunc(function(){
	...
	$(".r-fa-edit .fa-edit").on("click",function(){
		window.location.href= pagebase+"/auth/oper/role/update?id="+$(this).attr("rid");	
	});
   ...
   //批量删除
    $(".icon-trash-batch").on("click",function(){
    	...
    	rids =rids.substring(0, rids.length-1);
    	//window.location.href=pagebase+"/auth/oper/role/remove?ids="+rids;
    	wms.asyncPost(pagebase+"/auth/oper/role/remove", {"ids":rids},{successHandler:function(data, textStatus){
			window.location.href=pagebase +"/auth/oper/role/list";
    	}});
    });
    ...
    $(".role-trash-o").on("click",function(){
    	var me = $(this);
    	var rid=me.attr("rid");
    	wms.asyncPost(pagebase+"/auth/oper/role/remove", {"ids":rid},{successHandler:function(data, textStatus){
			window.location.href=pagebase +"/auth/oper/role/list";
    	}});
    });
