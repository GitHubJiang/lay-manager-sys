...
wms.addReadyFunc(function(){
	...
	$(".role-save").on("click",function(){
		...
		//获取id
		var id = $(".role .id").val();
		var url = pagebase+"/auth/oper/role/add";
		if(id!=null && id!=""){
			data.id=id;
			msg=i18n.t("role-update-s");
			url = pagebase+"/auth/oper/role/update";
		}
    ...
    /*$(".check-all").on("click", function() {
        ...
        if($(this).get(0).checked){
        	 $("input[data-role=" + target + "]", $(this).parents(".widget")).removeAttr("checked");
        }else{
        	$("input[data-role=" + target + "]", $(this).parents(".widget")).attr("checked","checked");
        }
        
    })
    $("input[data-role=all]").on("click", function(){
    	 if($(this).get(0).checked){
    ...
    $("input[data-target=all]").on("click", function(){
    	 if($(this).get(0).checked){
    		 $(this).parents("tr").find("input").removeAttr("checked");
    	 }else{
    		 $(this).parents("tr").find("input").attr("checked","checked");
